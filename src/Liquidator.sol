...
contract Liquidator is ReentrancyGuard, Ownable {
    ...
    function _processLiquidation(
        ...
        address effectiveCollateralToken = collateralToken == address(1) ? _wNative() : collateralToken;
    ...
    function _executeSwapAndRepay(
        ...
        uint256 amountOut = _performSwap(
            effectiveCollateralToken,
            borrowToken == address(1) ? _wNative() : borrowToken,
            collateralToLiquidate,
        ...
        address effectiveBorrowToken = borrowToken == address(1) ? _wNative() : borrowToken;
    ...
    function _executeMEVLiquidation(
        ...
        address effectiveToken = collateralToken == address(1) ? _wNative() : collateralToken;
        ...
        returns (uint256 amountOut)
    {
        ...
        IERC20(_tokenIn).approve(SWAP_ROUTER, amountIn);
    ...
    function _calculateCollateralForDebt(
        ...
        (, uint256 collateralPrice,,,) =
            IOracle(_tokenDataStream(collateralToken == address(1) ? _wNative() : collateralToken)).latestRoundData();
