...
contract Position is ReentrancyGuard {
    ...
    function withdrawCollateral(uint256 amount, address _user, bool unwrapToNative) public {
        ...
        if (_collateralToken() == address(1)) {
            if (unwrapToNative) {
                _withdrawing = true;
                IERC20(_wNative()).approve(_wNative(), amount);
                IWNative(_wNative()).withdraw(amount);
                (bool sent,) = _user.call{value: amount}("");
                if (!sent) revert TransferFailed();
                _withdrawing = false;
            } else {
                IERC20(_wNative()).safeTransfer(_user, amount);
            }
        } else {
        ...
        returns (uint256 amountOut)
    {
        ...
        amountOut = _performSwap(_tokenIn, _tokenOut, amountIn, slippageTolerance);
    ...
    function repayWithSelectedToken(uint256 amount, address _token, uint256 slippageTolerance) public payable {
        ...
        if (_token != _borrowToken()) {
            ...
            IERC20(_borrowToken() == address(1) ? _wNative() : _borrowToken()).safeTransfer(lpAddress, amount);
    ...
    function tokenCalculator(
        ...
        uint256 tokenInDecimal = _tokenIn == _wNative() ? 18 : IERC20Metadata(_tokenIn).decimals();
        uint256 tokenOutDecimal = _tokenOut == _wNative() ? 18 : IERC20Metadata(_tokenOut).decimals();
    ...
    function tokenValue(address token) public view returns (uint256) {
        ...
        if (token == address(1)) {
            tokenBalance = IERC20(_wNative()).balanceOf(address(this));
            tokenDecimals = 18;
        } else {
            // ERC20 token
    ...
    function _performSwap(address _tokenIn, address _tokenOut, uint256 amountIn, uint256 slippageTolerance)
        internal
        returns (uint256 amountOut)
    {
        amountOut = _attemptSwap(_tokenIn, _tokenOut, amountIn, slippageTolerance);
    }

    function _attemptSwap(address _tokenIn, address _tokenOut, uint256 amountIn, uint256 slippageTolerance)
        internal
        returns (uint256 amountOut)
    {
        address swapRouter = SWAP_ROUTER;
        ...
        IERC20(_tokenIn).approve(swapRouter, amountIn);

        // Prepare swap parameters
        ISwapRouter.ExactInputSingleParams memory params = ISwapRouter.ExactInputSingleParams({
            tokenIn: _tokenIn,
            tokenOut: _tokenOut == address(1) ? _wNative() : _tokenOut,
            fee: 1000,
    ...
    function _getTokenDecimals(address _token) internal view returns (uint256 decimals) {
        if (_token == address(1) || _token == _wNative()) {
            decimals = 18;
        } else {
